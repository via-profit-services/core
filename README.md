...
**Замечание:** Для того, чтобы ваше клиентское приложение автоматически передавало куки в запросе к GraphQL серверу, необходимо, чтобы `fetch` запрос содержал опцию `credentials` со значением `include` и был установлен заголовок `Access-Control-Allow-Origin` со значением `*`, например:
```ts
fetch('https://localhost:4000/auth/access-token', {
  method: 'POST',
  credentials: 'include',
  headers: {
...
Метод принимает Access токен и возвращает, в случае успеха, новую пару токенов 
...
Метод принимает токен аутентификации и возвращает, в случае успеха, payload 
