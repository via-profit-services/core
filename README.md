...
4. Всё время, которое отдельно хранится от даты, должно храниться в часовом поясе **UTC**
5. Каждая сущность, являющаяся элементом выборки, должна иметь две колонки: `createdAt` и `updatedAt` типа `timestamptz`
