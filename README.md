...
```bash
yarn add ssh://git@gitlab.com:via-profit-services/core.git#semver:^0.27.1
```
...
Сервер авторизации стартует на том же хосте, но имеет отличный endpoint (По умолчанию: `/auth`).
...
Аутентификация в GraphQL сервере позволяет лишь отозвать токен. Для получения токенов, валидации и рефреша необходимо использовать сервер авторизации (см. [Авторизация](#authorization))
