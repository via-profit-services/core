...
*Преобразует входные данные от GraphQL*
...
```ts
import { IContext, buildQueryFilter, IDirectionRange, TInputFilter } from '@via-profit-services/core';
...
*Формирует [GraphQL Cursor Connections](https://facebook.github.io/relay/graphql/connections.htm)*
...
### <a name="convention-databases"></a> Конвенция. Базы данных
 ...
 4. Формат представления валюты должен быть `bigint` или `numeric`. Денежные значения необходимо хранить в наменьшей денежной единице (в копейках, в центах и т.п.). Не использовать тип `money`, т.к. он чувствителен к локали и требует преобразования, которое может быть выполнено через `float`, что может привести к неверному округлению.
